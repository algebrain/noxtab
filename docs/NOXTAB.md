...
## Гигиена процессов (обязательно)
Перед запуском нового инстанса Chrome или dev-сервера нужно убедиться, что старый уже остановлен.

Почему это важно:
- можно случайно получить второй Chrome с другим профилем/сессией;
- можно занять тот же порт вторым сервером или подключиться не к той вкладке;
- автоматизация начинает работать по «не той» цели, что дает ложные результаты.

Правило:
1. Проверить, нет ли уже запущенных Chrome/dev-сервера для текущего сценария.
2. Если есть - остановить их.
3. Только после этого запускать новый инстанс.

Для агента это обязательный шаг перед `open/list/eval`, если в сценарии присутствует запуск процессов.

...
Подробная документация: [docs/NOXTAB.md](./docs/NOXTAB.md).
...
- `--port=9222` (default)
- `--target=<id>` to pick exact target id
- `--url-contains=<part>` to pick tab by URL substring
- `--title-contains=<part>` to pick tab by title substring

## Commands
- `list` list inspectable page targets
- `open <url>` navigate in selected tab
...
- `screenshot <path>` save PNG screenshot without overwrite; if exists, saves as `name-001.png`, `name-002.png`, ...
